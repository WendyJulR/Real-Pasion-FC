	...
	$(function () {
		...
		function attachFormValidator(elements) {
			...
			var regularConstraintsMessages = [
				...
				{
					type:       regula.Constraint.nombre,
					newMessage: "The nombre is not a valid nombre."
				},
				{
		...
		if (plugins.mailchimp.length) {
			for (i = 0; i < plugins.mailchimp.length; i++) {
				var $mailchimpItem = $(plugins.mailchimp[i]),
						$nombre = $mailchimpItem.find('input[type="nombre"]');

				// Required by MailChimp
				$mailchimpItem.attr('novalidate', 'true');
				$nombre.attr('name', 'nombre');

				$mailchimpItem.on('submit', $.proxy(function ($nombre, event) {
					event.preventDefault();
		...
		if (plugins.rdMailForm.length) {
			var i, j, k,
					msg = {
						...
						'MF003': 'Please, define nombre field in your form!',
